...
class Pulsardo extends events_1.EventEmitter {
    constructor(logger) {
        super();
        this.logger = logger;
        const pythonPath = atom.config.get('pulsardo.pythonPath') || 'python';
        const samplesDirectory = atom.config.get('pulsardo.samplesDirectory');
        let command = ['-m', 'pulsardo', '--pipe'];
        if (samplesDirectory !== '') {
        ...
        try {
            this.childProcess = (0, child_process_1.spawn)(pythonPath, command, {
                env: Object.assign(Object.assign({}, process.env), { SC3_PLUGINS: atom.config.get('pulsardo.useSC3Plugins')
                        ? '1'
                        : undefined }),
            });
            ...
            this.childProcess.on('error', (err) => {
                if (err.code === 'ENOENT') {
                    logger === null || logger === void 0 ? void 0 : logger.service(`Python was not found. Check that you have Python installed. You may need to give the full path to the Python executable in the Pulsardo package's settings.`, true);
                }
                logger === null || logger === void 0 ? void 0 : logger.service(err.toString(), true);
            });
            this.childProcess.on('close', (code) => {
                if (code) {
                    logger === null || logger === void 0 ? void 0 : logger.service(`Pulsardo has exited with code ${code}.`, true);
                }
                else {
